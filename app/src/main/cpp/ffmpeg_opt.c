...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    LOGE("avformat_open_input start file name is %s",filename);
    err = avformat_open_input(&ic, filename, file_iformat, &o->g->format_opts);
    LOGE("avformat_open_input result is %d",err);
    if (err < 0) {
        char buf[1024];
        av_strerror(ret,buf,1024);
        LOGE("avformat_open_input error is %s",buf);
                      ...
                      int (*open_file)(OptionsContext*, const char*))
{
    ...
    for (i = 0; i < l->nb_groups; i++) {
        ...
        LOGE("parse_optgroup result is %d",ret);
        if (ret < 0) {
        ...
        LOGE("inner open_file result is %d",ret);
        uninit_options(&o);
        if (ret < 0) {
...
int ffmpeg_parse_options(int argc, char **argv)
{
    ...
    LOGE("split_commandline result is %d",ret);
    if (ret < 0) {
    ...
    /* open input files */
    LOGE("start open file");
    ret = open_files(&octx.groups[GROUP_INFILE], "input", open_input_file);
    LOGE("open_files result is %d",ret);
    if (ret < 0) {
    ...
    /* open output files */
    LOGE("start open out file");
    ret = open_files(&octx.groups[GROUP_OUTFILE], "output", open_output_file);
    LOGE("open out file result %d",ret);
    if (ret < 0) {
